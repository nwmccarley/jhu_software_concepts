...
def create_app():
    app = Flask(__name__)
# Route for dashboard and displaying results from query_data.py
    @app.route("/")
    def dashboard():
        data = get_all_query_results()
        questions = [
            ("How many entries do you have in your database who have applied for Fall 2025?", data["q1"]),
            ...
            ("4. What is the average GPA of American students in Fall 2025?", data["q4"]),
            ("5. What percent of entries for Fall 2024 are Acceptances (to two decimal places)?", f"{data['q5']}%"),
            ("6. What is the average GPA of applicants who applied for Fall 2025 and were accepted?", data["q6"]),
